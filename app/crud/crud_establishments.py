...
def get_establishment(db: Session, establishment_id: int) -> Optional[Establishment]:
    return db.query(Establishment).filter(Establishment.establecimiento_id == establishment_id).first()
...
def get_establishments_by_user(db: Session, user_id: int, skip: int = 0, limit: int = 100) -> List[Establishment]:
    return db.query(Establishment).filter(Establishment.usuario_id == user_id).offset(skip).limit(limit).all()
